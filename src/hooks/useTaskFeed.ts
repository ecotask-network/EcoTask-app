...
export function useTaskFeed(options: UseTaskFeedOptions = {}) {
  const {
    tasks, isLoading, error, page, hasMore,
    setTasks, appendTasks, setLoading, setError, setPage, setHasMore,
  } = useTaskStore();
  ...
  }, [options.type, options.lat, options.lng, options.radius, setTasks, appendTasks, setLoading, setError, setPage, setHasMore]);

  const refresh = useCallback(() => loadTasks(1), [loadTasks]);
  const loadMore = useCallback(() => {
    if (!isLoading && hasMore) loadTasks(page + 1);
